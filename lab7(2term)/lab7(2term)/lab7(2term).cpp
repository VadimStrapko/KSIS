...
void Ping(const char* cHost,
    ...
    //Определяем данные эхозапроса в переменной
    ...
    unsigned long ipaddr = inet_addr(cHost); // преобразует айпишник

    IP_OPTION_INFORMATION option = { 250, 0, 0, 0, 0 };//для включения в заголовок IP пакета
    ...
    for (unsigned int c = 0; c < RequestCount; c++) {
        int dwStatus = IcmpSendEcho(hIP,
            ...
            &option,
            pIpe,
            sizeof(ICMP_ECHO_REPLY) * RequestCount +
            sizeof(SendData),
            Timeout);
...
int main()
{
    ...
    cout << "Ввод параметров для функции Ping " << endl;
    cout << "Введите Ip-адресс хоста: ";
    char ip[15];
    cin >> ip;
    cout << "Введите время ожидания ответа (в мс) после запроса: ";
    unsigned int Timeout;
    cin >> Timeout;
    cout << "Введите кол-во посылаемых запросов: ";
    unsigned int RequestCount;
    cin >> RequestCount;
